...
**Inherent Risk Score = B1 + B2 + B3 + B4 + B5 + B6 + B7 + B8 = ___ / 40**
...
Note: any single dimension scored 5 automatically escalates the band to at least High, regardless of total.
...
- Configure cross-border transfer safeguards (SCCs / adequacy / consent).
...
**If B4 Security ≥ 3**
- Threat-model for AI-specific attacks (prompt injection, poisoning, theft, jailbreak).
...
- For GenAI: define hallucination evaluation (see toolkit/07-genai-llm-annex.md).
...
| Dimension | Inherent (1–5) | Controls strength (Low / Med / High) | Residual (1–5) |
|---|---|---|---|
| B1 Impact | | | |
| B2 Privacy | | | |
| B3 Bias | | | |
...
**Residual Risk Score = sum of Residual column = ___ / 40** → Band: ____________________

### Approval authority based on residual band

| Residual band | Who must approve |
|---|---|
...
- Regulatory change
- 12 months elapsed (6 months for Tier 2 High-risk)
...
Update toolkit/01-ai-inventory.csv with the final Tier, residual band, approver, and next-assessment date. Roll this system's residual score and any open risks into the next quarterly toolkit/06-board-report.md.
